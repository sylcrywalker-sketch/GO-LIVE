Shader "GO LIVE/Surfaces/Measured Architectural Paint"
{
Properties
    {
    ...
    SubShader
    {
        ...
        // Narrow adapter over installed URP17.6 Lit passes. Architectural walls are flat, axis aligned faces.
        // One sample per map, vertex projection only: measured1m scale without changing source mesh/UV/colliders.
        HLSLINCLUDE
        #include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Core.hlsl"
        void ArchitecturalCoordinates(inout float2 uv, inout float4 tangentOS, float3 positionOS, float3 normalOS)
        {
            ...
            else if (a.x >= a.z)
            {
                float s = n.x >= 0 ? 1 : -1;
                uv = float2(-s * p.z, p.y);
                tangentWS = float3(0, 0, -s);
            }
            else
            {
                float s = n.z >= 0 ? 1 : -1;
                uv = float2(s * p.x, p.y);
                tangentWS = float3(s, 0, 0);
            }
            tangentWS = normalize(tangentWS - n * dot(n, tangentWS));
            tangentOS = float4(TransformWorldToObjectDir(tangentWS), GetOddNegativeScale());
        }
        ENDHLSL
        ...
        Pass
        {
            ...
            // -------------------------------------
            // Shader Stages
            #pragma vertex ArchitecturalVertex
            ...
            Varyings ArchitecturalVertex(Attributes input)
            {
                UNITY_SETUP_INSTANCE_ID(input);
                ArchitecturalCoordinates(input.texcoord, input.tangentOS, input.positionOS.xyz, input.normalOS);
...
Pass
        {
            ...
            // -------------------------------------
            // Shader Stages
            #pragma vertex ArchitecturalDepthNormalsVertex
            ...
            Varyings ArchitecturalDepthNormalsVertex(Attributes input)
            {
                UNITY_SETUP_INSTANCE_ID(input);
                ArchitecturalCoordinates(input.texcoord, input.tangentOS, input.positionOS.xyz, input.normal);
